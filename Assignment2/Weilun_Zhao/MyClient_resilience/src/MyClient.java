...
//        messagePrint("num of hashmap: " + theNumberOfHashMap);
	...
	private static boolean input(){
		...
		String value;
		int i;
		int clientNumber, clientNumber_backup;
		...
		clientNumber = getClientNumberWithInputKey(key);//set a client number using key
		messagePrint("the client number " + clientNumber);
		clientNumber_backup = getClientNumberWithInputKey_Resilience(key);//set a new back up client number
		
		String clientInfo = null;
		String clientInfo_backup = null;
		clientInfo = getClientInfo(clientNumber);
		clientInfo_backup = getClientInfo(clientNumber_backup);
		...
		i = contectedWithClient(clientInfo,key,value); //input key and value to expected client
		int i_r = contectedWithClient(clientInfo_backup,key,value); //input key and value to backup server
	...
	private static int get(){
		...
		if(clientInfo == ""){
			messagePrint("Not find client and try to get connect with back up server");
			
			//use back up server to get value using the same key
			int clientNumber_backup = getClientNumberWithInputKey_Resilience(key);
			String clientInfo_backup = getClientInfo(clientNumber_backup);
			if(clientInfo_backup == ""){
				return -1;	
			}
			clientInfo = clientInfo_backup;
		}
		
		value = sendKeyGetValue(clientInfo, key);
		if ( value == "" || value == null){
			messagePrint("Try to using back up server to get value");
			
			//using back up server to get value
			int clientNumber_backup = getClientNumberWithInputKey_Resilience(key);
			String clientInfo_backup = getClientInfo(clientNumber_backup);
			value = sendKeyGetValue(clientInfo_backup, key);
			if ( value == "" || value == null){
				return -1;
			}
		}
...
//		Socket socket = new Socket(InetAddress.getLocalHost(), port);
    ...
    private static int getClientNumberWithInputKey_Resilience(String key){
    	char k = key.charAt(0);
    	int i = theNumberOfHashMap - (k % theNumberOfHashMap)-1;
    	return i;
    }
